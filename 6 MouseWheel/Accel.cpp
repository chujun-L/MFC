...
BOOL CAccelView::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值

	BOOL bUp = TRUE;		// 先设定为向上滚动
	int nDelta = zDelta;

	if (zDelta < 0) {
		bUp = FALSE;
		nDelta = -nDelta;		// 将zDelta转换成正的
	}

	UINT nWheelScrollLines;	// 滚轮转动对应移动多少行(默认是3行)
	::SystemParametersInfo(SPI_GETWHEELSCROLLLINES, 0, &nWheelScrollLines, 0);

	int nLine = nDelta * nWheelScrollLines / WHEEL_DELTA;
